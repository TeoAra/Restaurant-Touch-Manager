<#
.SYNOPSIS
    HelloTable - Aggiornamento da GitHub
    Deve essere lanciato da aggiorna.bat (gia elevato come Amministratore)
#>
...
function Write-Warn { param($msg); Write-Host "    [ATTENZIONE] $msg" -ForegroundColor Yellow }
...
try {
    ...
    if (Test-Path $envFile) {
        ...
        Fail "File .env non trovato in $INSTALL_DIR. Reinstalla HelloTable."
    }

    Write-Host ""
    Write-Host "  HelloTable | Aggiornamento in corso..." -ForegroundColor DarkCyan
    ...
    if (-not (Test-Path $gitDir)) {
        ...
        & git init
        if ($LASTEXITCODE -ne 0) { Fail "git init fallito. Git e installato e nel PATH?" }
        & git remote add origin "https://github.com/TeoAra/Restaurant-Touch-Manager.git"
        if ($LASTEXITCODE -ne 0) { Fail "git remote add fallito." }
    }

    & git fetch --all
    if ($LASTEXITCODE -ne 0) { Fail "git fetch fallito. Controlla la connessione internet." }

    & git reset --hard origin/main
    if ($LASTEXITCODE -ne 0) { Fail "git reset fallito." }

    $lastCommit = & git log --oneline -1
    Write-Ok "Codice aggiornato: $lastCommit"

    # ── 3. Dipendenze ────────────────────────────────────────────────────────
    Write-Step "3/6  Aggiornamento dipendenze"
    & pnpm install --frozen-lockfile
    if ($LASTEXITCODE -ne 0) { Fail "pnpm install fallito." }
    Write-Ok "Dipendenze ok"

    # ── 4. Build ─────────────────────────────────────────────────────────────
    Write-Step "4a/6  Build API server"
    & pnpm --filter "@workspace/api-server" run build
    if ($LASTEXITCODE -ne 0) { Fail "Build API server fallita." }
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Migrazione database fallita." }
